...
class Form extends Tag  {
  ...
  const FORMMETHODS = ['get', 'post']; // allowed methods for form submission

  // Alert texts
  protected $alertSuccessText = ''; // alert text after successfull submission
  protected $alertErrorText = ''; // alert text if errors occur after submission
  ...
  protected $formElements = []; //array of form element objects
  protected $formErrors = []; // holds the array containing all form errors after submission

  protected $disableHoneypot = false; // Honeypotfield is enabled => true or disabled => false

  protected $removeInputWrapper = false; // Set or remove input wrapper for all form fields in general
  protected $removeFieldWrapper = false; // Set or remove field wrapper for all form fields in general

  protected $values = []; // array of all form values (key = name of the inputfield)

  protected $minTime = null; // min time that should be pass by before the form is submitted (spam protected) - default is null (no restriction)
  protected $maxTime = null; // max time until the form should be submitted (spam protected) - default is null (no restriction)
  protected $maxAttempts = null; // max number of attempts allowed until the form should be submitted valid
  ...
  protected $showForm = true; // show the form on the page
  protected $doubleSubmissionToken = ''; // the token for checking of double form submission; will be removed if form was valid
  ...
  */
  public function __construct(string $id)
  {
    ...
    $this->setAttribute('action', $this->wire('page')->url); // set the current page as default target after form submission
    ...
    $this->alert = new Alert(); // instantiate alert class
    // Set default messages for success and failure - can be overwritten
    $this->setSuccessMsg($this->_('Thank you for your message.')); // set default success message
    $this->setErrorMsg($this->_('Sorry, some errors occur. Please check your inputs once more.')); // set default error message
    $this->requiredHint = new RequiredTextHint(); // instantiate RequiredTextHint class
  ...
  */
  ...
  {
    $pass = new Password(); // instantiate a password object to use the methods
  ...
  */
  ...
  {
    return current(array_filter($this->formElements, function($e) use($name) { return $e->getAttribute('name') == $name; }));
  }
  ...
  */
  protected function encrypt_decrypt($string, $action = 'encrypt'): string
  {
      ...
      $secret_key = 'd0a7e7997b6d5fcd55f4b5c32611b87cd923e88837b63bf2941ef819dc8ca282'; // user define private key
      ...
      if(($this->getMinTime()) || ($this->getMaxTime())){
        ...
        if((($this->getMinTime()) || ($this->getMaxTime())) && (!$start_time)) {
          ...
          if($this->getMaxTime() && ($diff > $this->getMaxTime())){
            ...
            $this->wire('session')->set('blocked', $submit_Time); // set session for blocked value is the submission time
            return false;
  ...
  public function isValid()
  {
      ...
      foreach($formElements as $element){
        ...
        if(array_key_exists($fieldname, $input->getArray())){
          ...
          $element->setAttribute('value', $value); // add value to element
              ...
              if($this->checkDoubleFormSubmission($input)){
                ...
                if($this->wire('session')->CSRF->validate()){
                  ...
                  foreach($formElements as $element){
                    ...
                    if(!$this->disableHoneypot){
                      if($element->getAttribute('name') == 'seca'){
                        $v->rule('length', 'seca', 0)->message($this->_('Please do not fill out this field')); // length must be 0 = empty
                      }
                  ...
                  if($v->validate()) {
                    ...
                    $this->wire('session')->remove('attempts'); // remove attempt session
                    $this->wire('session')->remove('doubleSubmission');// remove the session for checking for double form submission
                    $this->showForm = false;
                    return true;
  ...
  */
  public function render(): string
  {
    ...
    $out .= $this->alert->render();// render the alert box on top for success or error message
    ...
    if(($this->showForm == true) && (($this->wire('session')->get('blocked') == null))){
      ...
      foreach($this->formElements as $element){
        ...
        if(is_subclass_of($element, 'FrontendForms\Inputfields')){
          ...
          if($element->getLabel()){
            $element->getLabel()->setAttribute('for', $element->getAttribute('id')); //set for attribute for the label tag
          }
        }
        if(array_key_exists($name, $this->formErrors)){
          $element->setCSSClass('input_errorClass'); // set error class for input element
          $element->setErrorMessage($this->formErrors[$name][0]);//get first error message
        }
